...
#ifdef ARDUINO
/* Arduinoが登録したベクターテーブル */
typedef void (* fsp_vector_t)(void);
extern fsp_vector_t *parduino_vector;
uint32_t tim_hoge;
...
target_timer_handler(void)
{
	...
	if ((parduino_vector[IRQNO_SYSTICK] != NULL) && (parduino_vector[IRQNO_SYSTICK] != Default_Handler)) {
		(*(parduino_vector[IRQNO_SYSTICK]))();
	}
}
